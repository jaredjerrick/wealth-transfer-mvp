...
def test_progressive_tax_uses_base_tax_correctly(ctx_mfj_250k):
    ...
    2026 brackets (Rev. Proc. 2025-32): MFJ 24% bracket runs $211,400 – $403,550.
    base_tax at $211,400 = $35,932; over = ($250,000 - $211,400) × 0.24 = $9,264.
    Total = $45,196.
    """
    brackets = ctx_mfj_250k.federal["income_tax_ordinary"]["mfj"]
    tax = progressive_tax(D("250000"), brackets)
    assert abs(tax - Decimal("45196.00")) < Decimal("0.50")
...
def test_annual_exclusion_2026_is_19000(ctx_mfj_250k):
    assert ctx_mfj_250k.annual_exclusion == Decimal("19000")


def test_basic_exclusion_2026_is_15000000(ctx_mfj_250k):
    """2026 §2010(c) basic exclusion is $15M per OBBBA §70106 / Rev. Proc. 2025-32."""
    assert ctx_mfj_250k.basic_exclusion_amount == Decimal("15000000")
...
def test_gift_within_exemption_no_tax(ctx_mfj_250k):
    ...
    With the 2026 BEA at $15M, after a $50K taxable gift, $14,950,000 remains.
    """
    ...
    assert ledger.gift_exemption_remaining == Decimal("14950000")
...
def test_federal_estate_tax_under_exemption_is_zero(ctx_mfj_250k):
    """An estate at the 2026 basic exclusion amount produces zero federal estate tax."""
    tax, _ = ctx_mfj_250k.federal_estate_tax(D("15000000"))
    assert tax == Decimal("0")


def test_federal_estate_tax_over_exemption(ctx_mfj_250k):
    """An estate $1M over the 2026 exemption is taxed at the 40% top marginal rate."""
    tax, _ = ctx_mfj_250k.federal_estate_tax(D("16000000"))
...
def test_portability_doubles_exclusion_when_spouse_present(ctx_mfj_250k):
    """With a surviving spouse and DSUE portability, 2 × $15M = $30M combined."""
    assert ctx_mfj_250k.applicable_exclusion_with_portability() == Decimal("30000000")
